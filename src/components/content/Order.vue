<template>
    ...
    ">
        ...
        <el-space :size="20" class="publisher-info">
            ...
            <span class="phone" v-show="store.state.user.userId == order.receiverId">{{ publisher.publisherContact }}</span>
        </el-space>
        <!-- 订单信息展示 -->
...
//     state.publisher.publisherName = res.name;
//     state.publisher.publisherIcon = res.userIcon;
//     state.publisher.publisherContact = res.userContact;
// })

let { publisher, orderCategoryName, orderStatusName } = toRefs(state)
